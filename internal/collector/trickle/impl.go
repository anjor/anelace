...
func (co *collector) FlushState() *anlblock.Header {
	...
	if co.UnixfsNulLeafCompat &&
		...
		return co.NodeEncoder.NewLink(nil)
	}
...
func (co *collector) AppendBlock(hdr *anlblock.Header) {
	...
	if co.tail == nil {
		...
		co.tail = &trickleNode{
			depth:        0,
			directLeaves: make([]*anlblock.Header, 1, co.MaxDirectLeaves+co.descentPrealloc),
			// this is a synthetic parent to hold the final-most-est digest CID
			parent: &trickleNode{
				depth:        -1,
				directLeaves: make([]*anlblock.Header, 0, 1),
			},
		}
